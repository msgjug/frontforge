...
out["win_designer"] = this.win_designer;
out["win_designer_w"] = this.win_designer_w;
out["win_designer_h"] = this.win_designer_h;
...
this.win_designer = input.hasOwnProperty("win_designer") && input["win_designer"] !== null ? input["win_designer"] : this.win_designer;
this.win_designer_w = input.hasOwnProperty("win_designer_w") && input["win_designer_w"] !== null ? input["win_designer_w"] : this.win_designer_w;
this.win_designer_h = input.hasOwnProperty("win_designer_h") && input["win_designer_h"] !== null ? input["win_designer_h"] : this.win_designer_h;
...
gBuf.writeBool (this.win_designer);
gBuf.writeInt32 (this.win_designer_w);
gBuf.writeInt32 (this.win_designer_h);
...
this.win_designer = gBuf.readBool ();
this.win_designer_w = gBuf.readInt32 ();
this.win_designer_h = gBuf.readInt32 ();
...
export class ProtocolObjectOpenFile extends Protocol {
    path : string = "";str : string = "";mode : string = "";
...
out["mode"] = this.mode;
    ...
    fromMixed(input: any) {
        ...
        this.path = input.hasOwnProperty("path") && input["path"] !== null ? input["path"] : this.path;
this.str = input.hasOwnProperty("str") && input["str"] !== null ? input["str"] : this.str;
this.mode = input.hasOwnProperty("mode") && input["mode"] !== null ? input["mode"] : this.mode;
...
gBuf.writeString (this.mode);
    ...
    protected _fromBinary( gBuf: GrowBuffer ) {
        ...
        this.path = gBuf.readString ();
this.str = gBuf.readString ();
this.mode = gBuf.readString ();

    }
};
export class ProtocolObjectSaveFile extends Protocol {
    path : string = "";str : string = "";silent : boolean = false;
...
export class ProtocolObjectPersistPrefab extends Protocol {
    prefab_conf : ProtocolObjectPrefabConfig = new ProtocolObjectPrefabConfig();

    getClassName(){
        return "PersistPrefab"
    }
    toMixed() {
        let out: any = {};
        out["__cn"] = this.getClassName();
...
}else if( msgName == "OpenFile") {
    return new ProtocolObjectOpenFile();
}else if( msgName == "SaveFile") {
    return new ProtocolObjectSaveFile();
}else if( msgName == "FlagPrefab") {
    return new ProtocolObjectFlagPrefab();
}else if( msgName == "PersistPrefab") {
